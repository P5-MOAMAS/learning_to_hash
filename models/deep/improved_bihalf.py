...
def get_config():
    config = {
        ...
        "optimizer": {"type": optim.SGD, "epoch_lr_decrease": 30,
                      "optim_params": {"lr": 0.001, "weight_decay": 5e-4, "momentum": 0.9}, "lr_type": "step"},
...
class BiHalfModelUnsupervised(nn.Module):
    def __init__(self, bit):
        ...
        self.model = ImprovedAlexNet(bit)
    ...
    def forward(self, x):
        x = self.model(x)  # Extract features via the updated AlexNet model
        h = x
...
def train_val(config, bit):
    ...
    net = BiHalfModelUnsupervised(bit).to(device)

    # Use SGD with learning rate adjustment
    optimizer = optim.SGD(net.parameters(), **(config["optimizer"]["optim_params"]))
    ...
    for epoch in range(config["epoch"]):
        ...
        print("\b\b\b\b\b\b\b\b loss:%.9f" % (train_loss))
